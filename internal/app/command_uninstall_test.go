...
func TestUninstallDryRun(t *testing.T) {
	...
	for _, expected := range []string{
		...
		"Built-in Agents: remove claude, codebuddy, codex, cursor, dcode, kiro, and workbuddy; remove managed config, logs, and state",
...
func TestUninstallRemovesAllBuiltInAdaptersAndManagedFiles(t *testing.T) {
	...
	for path := range hookFiles {
		...
		if os.IsNotExist(err) && strings.Contains(path, filepath.Join(".kiro", "hooks")) {
			continue
		}
		if err != nil {
			t.Fatal(err)

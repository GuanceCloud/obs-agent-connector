...
func uninstallConnector(args []string) error {
	...
	rows = append(rows, [2]string{"Built-in Agents", "remove claude, codebuddy, codex, cursor, dcode, grok, kiro, and workbuddy; " + managedFilesMode})
	if *keepConfig {
	...
	for _, adapter := range []string{"claude", "codebuddy", "codex", "cursor", "dcode", "grok", "kiro", "workbuddy"} {

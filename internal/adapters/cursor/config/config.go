...
func Resolve(options ResolveOptions) Config {
	...
	merged := map[string]any{
		...
		"resourceAttributes": map[string]any{
			"service.name": "gtrace-cursor", "telemetry.sdk.name": "gtrace",
			"telemetry.sdk.language": "go", "agent_runtime": "cursor",
		},
	}
...
func environmentConfig(env map[string]string) map[string]any {
	return clean(map[string]any{
		"enabled":            firstEnv(env, "CURSOR_OTEL_ENABLED", "TRACE_TO_GTRACE"),
		"endpoint":           firstEnv(env, "OTEL_EXPORTER_OTLP_ENDPOINT", "CURSOR_OTEL_ENDPOINT", "GTRACE_ENDPOINT"),
		"otel_traces_url":    firstEnv(env, "OTEL_EXPORTER_OTLP_TRACES_ENDPOINT", "CURSOR_OTEL_TRACES_ENDPOINT"),
		"otel_metrics_url":   firstEnv(env, "OTEL_EXPORTER_OTLP_METRICS_ENDPOINT", "CURSOR_OTEL_METRICS_ENDPOINT"),
		"tracePath":          firstEnv(env, "CURSOR_OTEL_TRACE_PATH", "GTRACE_TRACE_PATH"),
		"metricsPath":        firstEnv(env, "CURSOR_OTEL_METRICS_PATH", "GTRACE_METRICS_PATH"),
		"headers":            parseObject(firstEnv(env, "OTEL_EXPORTER_OTLP_HEADERS", "CURSOR_OTEL_HEADERS")),
		"resourceAttributes": parseObject(firstEnv(env, "OTEL_RESOURCE_ATTRIBUTES", "CURSOR_OTEL_RESOURCE_ATTRIBUTES")),
		"captureContent":     firstEnv(env, "CURSOR_OTEL_CAPTURE_CONTENT", "GTRACE_CAPTURE_CONTENT"),
		"maxChars":           firstEnv(env, "CURSOR_OTEL_MAX_CHARS", "GTRACE_MAX_CHARS"),
		"timeoutMs":          firstEnv(env, "CURSOR_OTEL_TIMEOUT_MS", "GTRACE_TIMEOUT_MS"),
		"debug":              firstEnv(env, "CURSOR_OTEL_DEBUG", "GTRACE_DEBUG"),
	})
}

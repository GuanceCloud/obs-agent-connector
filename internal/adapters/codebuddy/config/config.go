...
func Resolve(options ResolveOptions) Config {
	...
	merged := map[string]any{
		...
		"timeoutMs": DefaultTimeoutMs, "terminalWaitMs": DefaultTerminalWaitMs,
		"headers": map[string]any{},
		"resourceAttributes": map[string]any{
			"service.name": "gtrace-codebuddy", "telemetry.sdk.name": "gtrace",
			"telemetry.sdk.language": "go", "agent_runtime": "codebuddy",
		},
	}
	...
	if !hasEnabled {
		enabled = endpoint != "" || firstString(merged, "otel_traces_url", "tracesEndpoint") != ""
	}
	...
	return Config{
		...
		Transport: transport.Config{
			...
			TraceURL:   strings.TrimRight(firstString(merged, "otel_traces_url", "tracesEndpoint", "traceEndpoint"), "/"),
			MetricsURL: strings.TrimRight(firstString(merged, "otel_metrics_url", "metricsEndpoint", "metricEndpoint"), "/"),
			...
			Timeout:   time.Duration(integer(merged["timeoutMs"], DefaultTimeoutMs)) * time.Millisecond,
		},
...
func environmentConfig(env map[string]string) map[string]any {
	return clean(map[string]any{
		"enabled":            firstEnv(env, "CODEBUDDY_OTEL_ENABLED", "TRACE_TO_GTRACE"),
		"endpoint":           firstEnv(env, "OTEL_EXPORTER_OTLP_ENDPOINT", "CODEBUDDY_OTEL_ENDPOINT", "GTRACE_ENDPOINT"),
		"otel_traces_url":    firstEnv(env, "OTEL_EXPORTER_OTLP_TRACES_ENDPOINT", "CODEBUDDY_OTEL_TRACES_ENDPOINT"),
		"otel_metrics_url":   firstEnv(env, "OTEL_EXPORTER_OTLP_METRICS_ENDPOINT", "CODEBUDDY_OTEL_METRICS_ENDPOINT"),
		"tracePath":          firstEnv(env, "CODEBUDDY_OTEL_TRACE_PATH", "GTRACE_TRACE_PATH"),
		"metricsPath":        firstEnv(env, "CODEBUDDY_OTEL_METRICS_PATH", "GTRACE_METRICS_PATH"),
		"headers":            parseObject(firstEnv(env, "OTEL_EXPORTER_OTLP_HEADERS", "CODEBUDDY_OTEL_HEADERS")),
		"resourceAttributes": parseObject(firstEnv(env, "OTEL_RESOURCE_ATTRIBUTES", "CODEBUDDY_OTEL_RESOURCE_ATTRIBUTES")),
		"captureContent":     firstEnv(env, "CODEBUDDY_OTEL_CAPTURE_CONTENT", "GTRACE_CAPTURE_CONTENT"),
		"maxChars":           firstEnv(env, "CODEBUDDY_OTEL_MAX_CHARS", "GTRACE_MAX_CHARS"),
		"timeoutMs":          firstEnv(env, "CODEBUDDY_OTEL_TIMEOUT_MS", "GTRACE_TIMEOUT_MS"),
		"terminalWaitMs":     firstEnv(env, "CODEBUDDY_OTEL_TERMINAL_WAIT_MS"),
		"debug":              firstEnv(env, "CODEBUDDY_OTEL_DEBUG", "GTRACE_DEBUG"),
	})
}
...
func boolValue(value any) bool { result, _ := boolean(value); return result }

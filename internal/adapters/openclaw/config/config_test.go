...
func TestResolveUsesManagedConfigAndOpenClawEnvironment(t *testing.T) {
	home := t.TempDir()
	managedDir := filepath.Join(home, ".obs-agent-connector", "openclaw")
	...
	cfg := Resolve(ResolveOptions{Home: home, Env: map[string]string{
		...
		"OPENCLAW_OTEL_ENDPOINT": "https://env.example",
	}})
	...
	if cfg.ResourceAttributes["service.name"] != "gtrace-openclaw" || cfg.ResourceAttributes["agent_runtime"] != "openclaw" {
		t.Fatalf("missing OpenClaw resource defaults: %#v", cfg.ResourceAttributes)
	}
...
func TestResolveDisabledWithoutEndpoint(t *testing.T) {
	...
	if cfg.Enabled {
		t.Fatalf("empty OpenClaw configuration must be disabled: %#v", cfg)
	}
}

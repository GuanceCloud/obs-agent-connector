...
func Resolve(options ResolveOptions) Config {
	...
	merged := map[string]any{
		...
		"resourceAttributes": map[string]any{
			"service.name": "gtrace-openclaw", "telemetry.sdk.name": "gtrace",
			"telemetry.sdk.language": "go", "agent_runtime": "openclaw",
		},
	}
	for _, source := range []map[string]any{
		readJSON(filepath.Join(home, ".openclaw", "gtrace.json")),
		readJSON(configPath(env, home)),
	...
	if stateDir == "" {
		stateDir = filepath.Join(agentfiles.Directory(home, "openclaw"), "state")
	}
	return Config{
		Enabled: enabled,
		Transport: transport.Config{
		...
		MaxChars:           bounded(integer(merged["max_chars"], integer(merged["maxChars"], DefaultMaxChars)), 1, 100_000, DefaultMaxChars),
		Debug:              boolValue(merged["debug"]),
		LogFile:            agentfiles.HookLogPath(home, "openclaw"),
		StateDir:           stateDir,
	}
}
...
func environmentConfig(env map[string]string) map[string]any {
	return clean(map[string]any{
		"enabled":            firstEnv(env, "OPENCLAW_OTEL_ENABLED", "TRACE_TO_GTRACE"),
		"endpoint":           firstEnv(env, "OTEL_EXPORTER_OTLP_ENDPOINT", "OPENCLAW_OTEL_ENDPOINT", "GTRACE_ENDPOINT"),
		"otel_traces_url":    firstEnv(env, "OTEL_EXPORTER_OTLP_TRACES_ENDPOINT", "OPENCLAW_OTEL_TRACES_ENDPOINT"),
		"otel_metrics_url":   firstEnv(env, "OTEL_EXPORTER_OTLP_METRICS_ENDPOINT", "OPENCLAW_OTEL_METRICS_ENDPOINT"),
		"tracePath":          firstEnv(env, "OPENCLAW_OTEL_TRACE_PATH", "GTRACE_TRACE_PATH"),
		"metricsPath":        firstEnv(env, "OPENCLAW_OTEL_METRICS_PATH", "GTRACE_METRICS_PATH"),
		"headers":            parseObject(firstEnv(env, "OTEL_EXPORTER_OTLP_HEADERS", "OPENCLAW_OTEL_HEADERS")),
		"resourceAttributes": parseObject(firstEnv(env, "OTEL_RESOURCE_ATTRIBUTES", "OPENCLAW_OTEL_RESOURCE_ATTRIBUTES")),
		"captureContent":     firstEnv(env, "OPENCLAW_OTEL_CAPTURE_CONTENT", "GTRACE_CAPTURE_CONTENT"),
		"maxChars":           firstEnv(env, "OPENCLAW_OTEL_MAX_CHARS", "GTRACE_MAX_CHARS"),
		"timeoutMs":          firstEnv(env, "OPENCLAW_OTEL_TIMEOUT_MS", "GTRACE_TIMEOUT_MS"),
		"debug":              firstEnv(env, "OPENCLAW_OTEL_DEBUG", "GTRACE_DEBUG"),
		"stateDir":           firstEnv(env, "OPENCLAW_OTEL_STATE_DIR"),
	})
}
...
func configPath(env map[string]string, home string) string {
	if p := env["OPENCLAW_OTEL_CONFIG_FILE"]; p != "" {
		return p
	}
	return agentfiles.ConfigPath(home, "openclaw")
}

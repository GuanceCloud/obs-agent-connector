...
// Normalize maps Pi's per-model turns into one user-request Turn. Only the
// extension's terminal agent_settled snapshot reaches this function.
func Normalize(s Snapshot, cfg config.Config) (model.Turn, error) {
	if s.Schema != 1 || s.SessionID == "" || s.TurnID == "" || s.Start <= 0 || s.End < s.Start || s.End > time.Now().Add(24*time.Hour).UnixMilli() {
		return model.Turn{}, fmt.Errorf("invalid Pi terminal snapshot")
	}
	...
	for _, e := range s.Events {
		...
		case "llm_start":
			llmStart = e.At
			if e.CallID != "" {
				llmStarts[e.CallID] = e.At
			}
		case "first_token":
			if e.CallID != "" && firstTokens[e.CallID] == 0 {
				firstTokens[e.CallID] = e.At
			}
		case "assistant":
			m := e.Message
			...
			callID := e.CallID
			if callID == "" {
				callID = fmt.Sprintf("llm-%d", len(t.LLMCalls)+1)
			}
			ttft := float64(0)
			if first := firstTokens[callID]; first >= start {
				ttft = float64(first - start)
			}
			call := model.LLMCall{CallID: callID, StartUnixNano: a, EndUnixNano: b, Provider: m.Provider, RequestModel: m.Model, ResponseModel: m.Model, OutputMessages: messageList(m, true, cfg), OutputPreview: preview(textContent(m.Content)), Usage: usage, TTFTMs: ttft, FinishReasons: []string{finishReason(m.StopReason)}, Status: "ok"}
			...
			if m.StopReason == "error" || m.StopReason == "aborted" {
				call.Status = "error"
				call.ErrorType = "pi." + m.StopReason
		...
		case "tool_start":
			...
			tool := &model.ToolCall{CallID: e.CallID, SpanID: validHexID(e.SpanID, 8), Name: e.Name, StartUnixNano: a, Arguments: capture(e.Args), InputPreview: preview(e.Args), TriggeringLLMCall: trigger[e.CallID]}
		...
		case "tool_end":
			...
			if e.IsError {
				...
				tool.ErrorType = "pi.tool_error"
			}
			if tool.Skill != nil {
				tool.Skill.Status = skillStatus(tool.Status)
				tool.Skill.ErrorType = tool.ErrorType
				if cfg.CaptureContent != "none" {
					tool.Skill.Description, tool.Skill.Version = skillMetadata(resultText(e.Result), cfg.MaxChars)
				}
			}
			if e.Name == "subagent" {
				tool.ExtraAttributes = subagentAttributes(e.Result)
			}
		}
	}
	...
	if last.StopReason == "aborted" || s.TerminalStatus == "aborted" {
		...
		t.ErrorType = "pi.aborted"
	}
	if last.StopReason == "error" || s.TerminalStatus == "failed" {
		t.ErrorType = "pi.model_error"
	}
	...
	for _, id := range toolOrder {
		...
		if tool.EndUnixNano == 0 {
			...
			tool.ErrorType = "pi.incomplete_tool"
			if tool.Skill != nil {
				tool.Skill.Status = skillStatus(tool.Status)
				tool.Skill.ErrorType = tool.ErrorType
			}
...
func validHexID(value string, size int) string {
...
func subagentAttributes(value any) map[string]any {
	...
	attributes := map[string]any{"subagent.count": len(results)}
	failed := 0
	...
	for _, raw := range results {
		...
		if modelName, ok := result["model"].(string); ok && modelName != "" {
			models[modelName] = true
		}
	}
	attributes["subagent.failed_count"] = failed
	if len(models) > 0 {
		...
		sort.Strings(values)
		attributes["subagent.models"] = values
	}
	return attributes
}

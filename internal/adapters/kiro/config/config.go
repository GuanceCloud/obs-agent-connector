...
func Resolve(options ResolveOptions) Config {
	...
	merged := map[string]any{
		...
		"resourceAttributes": map[string]any{
			"service.name": "gtrace-kiro", "telemetry.sdk.name": "gtrace",
			"telemetry.sdk.language": "go", "agent_runtime": "kiro",
		},
	}
	for _, source := range []map[string]any{
		readJSON(filepath.Join(home, ".kiro", "gtrace.json")),
		readJSON(filepath.Join(cwd, ".kiro", "gtrace.json")),
		readJSON(agentfiles.ConfigPath(home, "kiro")),
	...
	if stateDir == "" {
		stateDir = filepath.Join(home, ".kiro", "gtrace")
	}
	...
	return Config{
		...
		LogFile:            agentfiles.HookLogPath(home, "kiro"),
		StateDir:           stateDir,
...
func environmentConfig(env map[string]string) map[string]any {
	return clean(map[string]any{
		"enabled":            firstEnv(env, "KIRO_OTEL_ENABLED", "TRACE_TO_GTRACE"),
		"endpoint":           firstEnv(env, "OTEL_EXPORTER_OTLP_ENDPOINT", "KIRO_OTEL_ENDPOINT", "GTRACE_ENDPOINT"),
		"otel_traces_url":    firstEnv(env, "OTEL_EXPORTER_OTLP_TRACES_ENDPOINT", "KIRO_OTEL_TRACES_ENDPOINT"),
		"otel_metrics_url":   firstEnv(env, "OTEL_EXPORTER_OTLP_METRICS_ENDPOINT", "KIRO_OTEL_METRICS_ENDPOINT"),
		"tracePath":          firstEnv(env, "KIRO_OTEL_TRACE_PATH", "GTRACE_TRACE_PATH"),
		"metricsPath":        firstEnv(env, "KIRO_OTEL_METRICS_PATH", "GTRACE_METRICS_PATH"),
		"headers":            parseObject(firstEnv(env, "OTEL_EXPORTER_OTLP_HEADERS", "KIRO_OTEL_HEADERS")),
		"resourceAttributes": parseObject(firstEnv(env, "OTEL_RESOURCE_ATTRIBUTES", "KIRO_OTEL_RESOURCE_ATTRIBUTES")),
		"captureContent":     firstEnv(env, "KIRO_OTEL_CAPTURE_CONTENT", "GTRACE_CAPTURE_CONTENT"),
		"maxChars":           firstEnv(env, "KIRO_OTEL_MAX_CHARS", "GTRACE_MAX_CHARS"),
		"timeoutMs":          firstEnv(env, "KIRO_OTEL_TIMEOUT_MS", "GTRACE_TIMEOUT_MS"),
		"debug":              firstEnv(env, "KIRO_OTEL_DEBUG", "GTRACE_DEBUG"),
		"stateDir":           firstEnv(env, "KIRO_OTEL_STATE_DIR"),
		"sessionDir":         firstEnv(env, "KIRO_SESSION_DIR"),
	})
}

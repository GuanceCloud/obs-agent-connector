...
import (
	...
	"github.com/GuanceCloud/obs-agent-connector/internal/adapters/kiro/buildinfo"
	kiroconfig "github.com/GuanceCloud/obs-agent-connector/internal/adapters/kiro/config"
	kiroparse "github.com/GuanceCloud/obs-agent-connector/internal/adapters/kiro/parse"
...
type queuedTurn struct {
	...
	Events            []kiroparse.JournalEvent `json:"events,omitempty"`
	Turn              *model.Turn              `json:"turn,omitempty"`
}

type RunOptions struct {
	Config     *kiroconfig.Config
...
func RunCLI(args []string) int {
	...
	cwd, _ := os.Getwd()
	cfg := kiroconfig.Resolve(kiroconfig.ResolveOptions{Cwd: cwd})
	if !cfg.Enabled {
		return 0
	}
	...
	if payloadCwd := stringValue(payload, "cwd"); payloadCwd != "" {
		cfg = kiroconfig.Resolve(kiroconfig.ResolveOptions{Cwd: payloadCwd})
		if !cfg.Enabled {
			return 0
		}
	}
...
func RecordEvent(event string, payload map[string]any, cfg kiroconfig.Config) error {
	if !cfg.Enabled {
		return nil
	}
	event = firstNonEmpty(event, stringValue(payload, "hook_event_name", "hookEventName"))
	if event == "" {
		return errors.New("Kiro Hook event is required")
	}
	sessionID := stringValue(payload, "session_id", "sessionId")
	if sessionID == "" {
		return errors.New("Kiro Hook payload is missing session_id")
	}
	...
	stored := kiroparse.JournalEvent{Event: event, RecordedNano: time.Now().UnixNano(), Payload: payloadForStorage(payload, cfg)}
...
func enqueueStop(payload map[string]any, cfg kiroconfig.Config) (string, error) {
	storedPayload := payloadForStorage(payload, cfg)
	sessionID := stringValue(storedPayload, "session_id", "sessionId")
	if sessionID == "" {
		return "", errors.New("Kiro Stop payload is missing session_id")
	}
	...
	queued := queuedTurn{
		SessionID: sessionID, Cwd: stringValue(storedPayload, "cwd"), Events: events,
		AssistantResponse: stringValue(storedPayload, "assistant_response", "assistantResponse"),
		AgentVersion:      stringValue(storedPayload, "version", "kiro_version"),
	}
...
func startPendingWorkers(executable string, cfg kiroconfig.Config) error {
	queueDir := filepath.Join(cfg.StateDir, "queue")
	...
	for _, entry := range entries {
		...
		command := exec.Command(executable, "hook", "kiro", "worker", path)
...
func ProcessQueue(queuePath string, options RunOptions) error {
	...
	cfg := kiroconfig.Resolve(kiroconfig.ResolveOptions{Cwd: queued.Cwd})
...
func waitForTurn(queued queuedTurn, cfg kiroconfig.Config, skipWait bool) (model.Turn, bool, error) {
	deadline := time.Now().Add(2 * time.Second)
	var lastErr error
	for {
		turn, ok, err := kiroparse.ReadLatestTurn(kiroparse.Options{
			SessionDir: cfg.SessionDir, SessionID: queued.SessionID, Cwd: queued.Cwd,
			AssistantResponse: queued.AssistantResponse, AgentVersion: queued.AgentVersion,
...
func exportTurn(cfg kiroconfig.Config, turn model.Turn, httpClient *http.Client) error {
	spans := (semantic.Builder{ScopeName: "gtrace-kiro-collector", ScopeVersion: buildinfo.Version}).Build(turn)
	if len(spans) == 0 {
		return nil
	}
	...
	if err != nil {
		return fmt.Errorf("claim Kiro turn: %w", err)
	}
	completed := false
	defer func() {
		if !completed {
...
func payloadForStorage(payload map[string]any, cfg kiroconfig.Config) map[string]any {
	metadataKeys := []string{
		"hook_event_name", "hookEventName", "session_id", "sessionId", "cwd", "tool_name", "toolName",
		"tool_use_id", "toolUseId", "tool_call_id", "toolCallId", "version", "kiro_version", "is_error", "isError",
	}
	contentKeys := []string{"prompt", "assistant_response", "assistantResponse", "tool_input", "toolInput", "tool_response", "toolResponse", "tool_output", "toolOutput"}
	out := map[string]any{}
...
func appendJournal(path string, event kiroparse.JournalEvent) error {
...
func readJournal(path string) ([]kiroparse.JournalEvent, error) {
	file, err := os.Open(path)
	if err != nil {
		return nil, err
	}
	defer file.Close()
	events := make([]kiroparse.JournalEvent, 0)
	scanner := bufio.NewScanner(file)
	scanner.Buffer(make([]byte, 64*1024), 4*1024*1024)
	for scanner.Scan() {
		var event kiroparse.JournalEvent
...
func journalPath(stateDir, sessionID string) string {
	return filepath.Join(stateDir, "journal", derivedID("kiro", sessionID)+".jsonl")
}
...
func appendLog(cfg kiroconfig.Config, message string, extra map[string]any) {

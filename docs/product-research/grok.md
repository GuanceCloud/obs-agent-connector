...
| Turn outcome | `TurnCompleted.stop_reason`, `agent_result`, and optional aggregate usage | Turn | Authoritative terminal evidence; aggregate usage is retained internally and can be apportioned across an exact matching set of calls with explicit estimated provenance |
...
| schema `1.0` event turn | `LLMCall` | `llm` | Real phase boundary and optional TTFT; a complete matching aggregate may be proportionally allocated with estimated provenance |
| validated aggregate usage plus tool clusters | `LLMCall` | `llm` | Explicitly synthetic estimated timing; a complete matching aggregate may be proportionally allocated with estimated provenance |
...
| Response records absent on a non-Messages backend | Exact per-call token usage remains unavailable | Use unambiguous schema `1.0` event timing when available; conservatively apportion a complete matching turn aggregate and mark it estimated | Adopt a future public per-call usage record |
| Agent Monitoring reads token fields from `llm` spans | Multi-call traces need LLM-level values even though Grok exposes only an exact turn aggregate | Use conservation-based estimated allocation across the validated call set; omit aggregate usage from the root | Prefer native per-response usage when Grok exposes it |

...
| `cursor` | Cursor with automatic `~/.cursor` or Cursor CLI-family detection, preferring `cursor-agent` | Unix: `https://static.guance.com/cursor-otel-plugin/install.sh`  Windows: `https://github.com/GuanceCloud/cursor-otel-plugin/releases/latest/download/install-release.ps1` | `~/.cursor/gtrace.json` | `~/.cursor/hooks/cursor-otel-plugin` |

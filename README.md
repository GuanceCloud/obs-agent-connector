...
The tool provides one binary and one version for connector lifecycle operations and built-in telemetry adapters for Claude, CodeBuddy, Codex, Cursor, Deep Agents Code, Grok Build, Kiro CLI, OMP, Pi, and WorkBuddy. Other Agents continue to use their external plugins.

## Features

- Bootstrap the CLI and OBS defaults with one installer command.
- Collect Claude, CodeBuddy, Codex, Cursor, Deep Agents Code, Grok Build, Kiro CLI, OMP, Pi, and WorkBuddy turns through built-in adapters without separate repositories.
...
| `pi` | Built into `obs-agent-connector` | `✅` | `✅` | `✅` | Pi 0.85.1+ native lifecycle extension; synthetic validation pending product smoke |
...
- On Windows, `claude`, `codebuddy`, `codex`, `cursor`, `dcode`, `dsh`, `grok`, `kiro`, `omp`, `opencode`, `openclaw`, `pi`, `qoder`, and `workbuddy` are supported.
- Claude, CodeBuddy, Codex, Cursor, Deep Agents Code, Grok Build, Kiro, OMP, Pi, and WorkBuddy register the connector directly; external plugins use the PowerShell installer from the configured OSS or GitHub source.
...
Claude, CodeBuddy, Codex, Cursor, Deep Agents Code, Grok Build, Kiro, OMP, Pi, and WorkBuddy are managed as built-in adapters with ordinary commands such as `install <agent>`, `status <agent>`, and `remove <agent>`.
...
`config` currently supports the managed `gtrace.json` layout used by `claude`, `codebuddy`, `codex`, `cursor`, `dcode`, `grok`, `kiro`, `omp`, `pi`, `opencode`, `qoder`, and `workbuddy`. `hermes` and `openclaw` are excluded.
